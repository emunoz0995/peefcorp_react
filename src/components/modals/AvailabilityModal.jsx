...
// SLICE
import { createAvailabilityThunk, initialStateAvailability } from '../../store/slices/availability/proceduresAvailability.slice'
//RESOURCE 
...
const AvailabilityModal = () => {
    ...
    const onSubmit = (data) => {
        ...
        dispatch(createAvailabilityThunk(formData));
    };
    ...
    if (availabilityState.message === 'resource created successfully') {
        reset();
    }

    return (
        <>
            <BtnTable action="pluss" funtion={() => handleOpenModal(true)} />
                ...
                <div className="modal-box p-5 max-w-xl max-h-[320px] contenedor mt-[120px] bg-white rounded-md">
                    <button onClick={() => handleOpenModal(false)} className="z-20 border-none text-gray-700 btn btn-sm btn-circle bg-rose-200  hover:bg-rose-400 absolute right-2 top-2">✕</button>
                    {
                        availabilityState.error ?
                            ...
                            :
                            availabilityState.message === 'resource created successfully' ?
                                <div className='flex flex-col p-2 w-full text-gray-800 border-green-500 rounded-md border mt-6 bg-green-200'>
                                    <p className='mb-0'>{availabilityState.message}</p>
                                </div>
                                : ' '
                    }

                    <form onSubmit={handleSubmit(onSubmit)} className='text-gray-700'>
                        <HeaderForm title={"New Availability"} />
                        <div className='flex gap-2 p-2'>
                            <DropdownForm
                                ...
                                options={years}
                                errors={errors.year && (<span className="text-red-500 text-xs">{"required information"}</span>)}
                            />
                        </div>
                        <div className='flex gap-2 p-2'>
                            <InputForm
